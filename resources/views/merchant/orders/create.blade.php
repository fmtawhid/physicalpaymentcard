    ...
    <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <div class="grid items-start gap-6 lg:grid-cols-[minmax(0,1.45fr)_minmax(320px,.7fr)]">
            ...
            <aside class="space-y-6 lg:sticky lg:top-24">
                <section class="overflow-hidden rounded-3xl bg-[#21130f] p-6 text-white shadow-2xl sm:p-7"><div class="flex items-start justify-between"><div><p class="text-xs font-black uppercase tracking-[.18em] text-orange-200">Your selection</p><h2 class="mt-2 text-2xl font-black">{{ $product->name }}</h2></div><span class="grid h-11 w-11 place-items-center rounded-2xl bg-orange-500 text-xl font-black">$</span></div><div class="mt-7 rounded-2xl bg-gradient-to-br from-orange-500 to-amber-300 p-5 text-slate-950 shadow-xl"><div class="flex items-start justify-between"><span class="text-xl font-black tracking-[-.08em]">payoneer</span><span class="text-lg font-black italic">mc</span></div><div class="mt-8 h-8 w-11 rounded-lg bg-gradient-to-br from-yellow-100 to-orange-100"></div><p class="mt-6 font-mono text-lg tracking-[.2em]">•••• 4821</p><div class="mt-5 flex justify-between text-[10px] font-black uppercase tracking-widest text-slate-900/60"><span>Virtual Mastercard</span><span>Secure</span></div></div><div class="mt-6 flex items-end justify-between border-t border-white/10 pt-5"><span class="text-sm text-orange-100/65">পরিশোধযোগ্য</span><strong class="text-2xl font-black">{{ $product->currency }} {{ number_format((float) $product->price, 2) }}</strong></div></section>
