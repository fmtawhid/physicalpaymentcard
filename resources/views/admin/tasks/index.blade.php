...
<main class="flex-1 overflow-y-auto p-6 bg-gray-50">
    <div class="flex justify-between items-center mb-6">
        <h2 class="text-2xl font-bold text-gray-800">Tasks for Project: {{ $project->title }}</h2>
        <a href="{{ route('admin.project.tasks.create', $project) }}" 
            ...
            <tbody class="bg-white divide-y divide-gray-200">
                ...
                <tr class="hover:bg-gray-50 transition duration-150">
                    ...
                    <td class="px-6 py-4 flex space-x-2">
                        <a href="{{ route('admin.project.tasks.edit', [$project, $task]) }}" 
                           class="px-3 py-1 bg-blue-500 hover:bg-blue-600 text-white text-sm rounded-md transition duration-300">
                           Edit
                        </a>
                        <form method="POST" action="{{ route('admin.project.tasks.destroy', [$project, $task]) }}" style="display: inline;">
                            @csrf
                            @method('DELETE')
